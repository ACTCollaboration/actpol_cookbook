...
\ba
\tilde{C}(\bm{\ell})= \rho(\bm{\ell}) \tilde{T}_{i}(\bm{\ell}) \tilde{T}^{*}_{j}(\bm{\ell})
\ea
Where we have introduced $\rho(\bm{\ell})$ which is an arbitrary weight function that can be used to give different weight to different modes (it can be the k-mask, or an azimuthal weight). However this estimator is biased $\langle \tilde{C}(\bm{\ell}) \rangle \neq C(\bm{\ell})$, indeed
\ba
\langle \tilde{C}(\bm{\ell}) \rangle &=& \langle  \tilde{T}_{i}(\bm{\ell}) \tilde{T}^{*}_{j}(\bm{\ell}) \rangle \\
&=& \rho(\bm{\ell}) \int d\bm{\ell'} | W (\bm{\ell}-\bm{\ell'})|^{2}b^{2}(\bm{\ell'})  C(\bm{\ell'})
\ea
...
\newcommand{\costwo}{\ensuremath{%
  \mathchoice{\includegraphics[height=4ex]{cos2.png}}
...
\newcommand{\sintwo}{\ensuremath{%
  \mathchoice{\includegraphics[height=4ex]{sin2.png}}
...
\ea
At the end, the expression of the mode coupling is simple of can be computed with $\ell_{\rm max}^{3}$ operations. 
\ba
...
\ea
We would like to switch to an estimator allowing for different weighting of the spherical harmonic
\ba
